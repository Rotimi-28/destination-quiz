...
async function loginFormHandler(event) {
    ...
    if (user && password) {
      const response = await fetch( `https://blooming-escarpment-07246.herokuapp.com/api/users/login` || 'http://localhost:3001/api/users/login', {
        method: 'post',
        body: JSON.stringify({
          user,
          password
...
async function signupFormHandler(event) {
    ...
    if (username && password && email) {
        ...
        const response = await fetch(`https://blooming-escarpment-07246.herokuapp.com/api/users` || 'http://localhost:3001/api/users', {
            method: 'post',
            body: JSON.stringify({
                username,
                email,
